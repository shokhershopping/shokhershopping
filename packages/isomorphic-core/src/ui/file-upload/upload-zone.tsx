...
interface UploadZoneProps {
  ...
  className?: string;
  error?: string;
  onUploadedFileChange?: (fileChanges: any) => void;
}
...
export default function UploadZone({
  ...
  useEffect(() => {
    if (onUploadedFileChange) {
      onUploadedFileChange(getValues(name));
    }
  }, [uploadedFiles]);
  ...
  async function handleRemoveUploadedFile(index: number) {
    setIsUploading(true);
    try {
      ...
      setUploadedFiles(
        uploadedFiles.filter((file) => file.filename !== fileToDelete.fileName)
      );
          ...
          {uploadedItems.map((file: any, index: number) => (
            <div key={index} className={cn("relative")}>
              <figure className="group relative h-40 rounded-md bg-gray-50">
                ...
                <button
                  type="button"
                  className="absolute left-0 top-0 rounded-full bg-gray-700 p-1.5 transition duration-300"
                >
